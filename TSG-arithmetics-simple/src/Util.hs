...
data FDef  = DEFINE FName [Parm] Term deriving ( Show)
...
data Cond = EQA' AExp AExp
          | CONS' Exp EVar EVar AVar deriving (Show)
      ...
      (ALT (CONS' e_str e_str_head e_str_tail a__)
        (ALT (CONS' e_str_head e__ e__ a_str_head)
          ...
          (ALT (EQA' a_subs_head a_str_head)
            (CALL "CheckPos"[e_subs_tail,e_str_tail,
...
data InEq  = CExp :=/=: CExp deriving (Show)
data Restr = INCONSISTENT | RESTR [InEq] deriving (Show)
...
--------------
data SBind = CVar :-> CExp deriving (Show, Eq)
...
data Clash = CExp :=: CExp deriving (Eq, Show)
type Clashes = [Clash]
...
instance Show Tree where
  show (LEAF c)    = "{" ++ show c ++ "}\n\t"
  show (NODE c bs) = "[ (" ++ show c ++ "): " ++ (intercalate " - " $ map show bs) ++ " ]\n"
...
----------
-- xptr without double tree visiting
-- I just inserted cutT to evalT (see evalTcut)
-- so we apply cutT not to the tree after ptr, but inside it and apply it to 1-2 item lists

xptr :: Prog -> Class -> Tree
xptr p cl@(ces, r) = evalTcut c p i
    where
      (DEFINE f prms _) : p' = p
      ce = mkEnv prms ces
      c = ((CALL f prms, ce), r)
...
evalTcut ::Conf -> Prog -> FreeIndex -> Tree
evalTcut c@((CALL f args , ce), r) p i =
	 NODE c $ cutT [ (idC, evalTcut c' p i) ]
	 where DEFINE _ prms t' = getDef f p
	       ce' = mkEnv prms (args /. ce)
               c' = ((t', ce'), r)
evalTcut c@((ALT cnd t1 t2, ce), r) p i =
         NODE c $ cutT [(cnt1, evalTcut c1' p i'),(cnt2, evalTcut c2' p i')]
         where ((cnt1, cnt2), uce1, uce2, i') = ccond cnd ce i
               ((_,ce1),r1) = c /. cnt1
               c1' = ((t1, ce1 +. uce1), r1)
               ((_, ce2),r2) = c /. cnt2
               c2' = ((t2, ce2 +. uce2), r2)
evalTcut c@((RETURN exp, ce), r) p i = LEAF c
...
type Tab    = [(Class, CExp)]
tab :: Prog -> Class -> Tab
tab p x = tab' [(x,tree)] [] where tree = xptr p x
...
subClassCntr (ces, r) (cesi, ri) = (cesr, rr)
  where
    (True, cesr) = unify ces cesi
    rr = case ri of
      INCONSISTENT -> INCONSISTENT
      RESTR rsi -> case r of
        INCONSISTENT -> RESTR rsi
        RESTR rs -> RESTR $ rsi \\ (rs /. cesr)

---------- Util for arithmetic operations
zero = ATOM "0"
one = ATOM "1"
empty = ATOM ""
failure = ATOM "FAIL"

decimalToBinaryExp :: Int -> Exp
decimalToBinaryExp n = decimalToBinaryExp' n empty

decimalToBinaryExp' :: Int -> Exp -> Exp
decimalToBinaryExp' 0 acc = CONS (ATOM "0") acc
decimalToBinaryExp' 1 acc = CONS (ATOM "1") acc
decimalToBinaryExp' n acc = decimalToBinaryExp' (n `div` 2) (CONS (ATOM $ show rem) acc)
                         where rem = n `mod` 2

binaryExpToDecimal :: Exp -> Int
binaryExpToDecimal exp = binaryExpToDecimal' exp 0

binaryExpToDecimal' :: Exp -> Int -> Int
binaryExpToDecimal' (ATOM _) acc = acc
binaryExpToDecimal' (CONS (ATOM h) t) acc = binaryExpToDecimal' t (acc * 2 + hInt)
                                          where hInt = read h :: Int